        ...
        for word in sentences:
            ...
            for val in v:
                if (val=="JJ" or val=="JJR" or val=="JJS" or val=="VB" or val=="VBD" or val=="VBG"
                    or val=="VBN" or val=="VBP" or val=="VBZ" or val=="RB" or val=="RBR" or val=="RBS" or val=="NN"):
                    altered_text=altered_text+stemmer.stem(word[0])+" "
    # print("cleaner poora chala")
    print("ye raha ahamara text after cleaner",altered_text)
    return altered_text
...
class FindSentiment(APIView):
    def post(self, request, format=None):
        ...
        json_file = open('../modelsentif (1).json', 'r')
        # print("model aa gaya")
        ...
        loaded_model.load_weights("../modelsentif (1).h5")
        print("model loaded from disk")
        if anss:
            # output_emots=[]
            for ans in anss:
                createfeed=pd.read_csv('../sentfinallexicon (1).csv',index_col=0)
    ...
    def post(self, request, format=None):
        ...
        for file in audios:
            ...
            with harvard as source:
                ...
                output_emots.append(output_emot)
                anss.append(ans)
        return Response([output_emots,anss])
        return Response(status=status.HTTP_404_NOT_FOUND)
...
class ChatterBot(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        dif={}
        if (len(quesans3)) & (c==5):
            print("aaya ander sabse pehle k liye")
            i=0
            ll=[]
            for x in quesans3:
        ...
        if a==5:
            a=a+1
            print("are you entering here",index)
